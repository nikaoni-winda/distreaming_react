...
import { getActorById, updateActor, deleteActor, addMovieToActor, removeMovieFromActor } from '../../services/actorService';
...
function EditActor() {
    ...
    const fetchActor = async () => {
        try {
            ...
            showToast('Error fetching actor: ' + (error.message || 'Unknown error'), 'error');
        } finally {
            setLoading(false);
        }
    };
    ...
    const searchMovies = async () => {
        try {
            ...
            // Filter out movies already in the actor's list
            const currentMovieIds = new Set(actor?.movies?.map(m => m.movie_id) || []);
    ...
    const handleUpdate = async (e) => {
        ...
        setSaving(true);
        try {
            await updateActor(id, { actor_name: actorName });
            showToast(t('admin.actorUpdated'), 'success');
            navigate('/admin/dashboard?tab=actors');
        } catch (error) {
    ...
    const handleDelete = () => {
        setConfirmModal({
            isOpen: true,
            title: t('admin.deleteActor') || 'Delete Actor',
            message: t('admin.deleteActorConfirm') || 'Are you sure you want to delete this actor?',
            isDestructive: true,
            onConfirm: async () => {
                try {
                    await deleteActor(id);
                    showToast(t('admin.actorDeleted') || 'Actor deleted successfully', 'success');
                    navigate('/admin/dashboard?tab=actors');
                } catch (error) {
    ...
    const handleAddMovie = async (movieId) => {
        try {
            await addMovieToActor(id, movieId);
            // Refresh
            const data = await getActorById(id);
            setActor(data);
            ...
            showToast(t('admin.movieAddedToActor'), 'success');
        } catch (error) {
    ...
    const handleRemoveMovie = (movieId) => {
        setConfirmModal({
            isOpen: true,
            title: t('admin.removeMovieFromActor') || 'Remove Movie',
            message: t('admin.removeMovieFromActorConfirm') || 'Are you sure you want to remove this movie from the actor?',
            isDestructive: true,
            onConfirm: async () => {
                try {
                    await removeMovieFromActor(id, movieId);
                    // Refresh
                    const data = await getActorById(id);
                    setActor(data);
                    showToast(t('admin.movieRemovedFromActor'), 'success');
                } catch (error) {
        ...
        <div className="min-h-screen bg-black flex flex-col md:flex-row font-sans text-white">
            ...
            <main className="flex-1 md:ml-64 p-6 md:p-8">
                ...
                <div className="flex justify-between items-center mb-8">
                    <button
                        onClick={() => navigate('/admin/dashboard?tab=actors')}
                ...
                <div className="max-w-4xl mx-auto">
                    <h1 className="text-3xl font-bold mb-8">{t('admin.editActor')}</h1>
                    ...
                    <div className="mt-12">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            >
                                <FaPlus /> {t('admin.addMovieToActor')}
                            </button>
                        </div>
                                        ...
                                        <div className="aspect-[2/3] relative bg-black">
                                            ...
                                            <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition flex items-center justify-center">
                                                <button
                                                    ...
                                                    title={t('admin.removeMovieFromActor')}
                                                >
                                                    <FaTrash />
                                                </button>
                                            </div>
